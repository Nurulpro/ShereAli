      ...
      <div class="sl-pagebody">
         	...
         	<div class="col-md-6">
         	    <div class="card">
         	    	...
         	    	<table class="table">
                          ...
                          <span class="badge badge-info">Cancled Order</span>
         ...
         <div class="row">
       	  ...
       	  @if($order->status_code == 0)
              ...
              <a href="{{ url('admin/payment/cancel/'.$order->id) }}" class="btn btn-danger" id="delete">Cancle Order</a>

          @elseif($order->status_code == 1)
              <a href="{{ url('admin/delevery/progress/'.$order->id) }}" class="btn btn-info">Progress Order</a>
              <strong> Payment Already Checked and pass here for delevery request</strong>

            @elseif($order->status_code == 3)
            <a href="{{ url('admin/delevery/done/'.$order->id) }}" class="btn btn-success">Delevered Unit</a>
               <strong> Payment Already done this product  are should Delevery unit </strong>

          @elseif($order->status_code == 2)
            <strong class="text-danger">This order are not valid or canceled</strong>
              @elseif($order->status_code == 4)
           
              <a href="{{ url('admin/handover/done/'.$order->id) }}" class="btn btn-success">Handover Done</a>
              @else
                <strong class="text-success">This product are succesfully hand over</strong>
            @endif
